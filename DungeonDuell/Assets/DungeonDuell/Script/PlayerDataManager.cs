...
namespace dungeonduell
{
    ...
    public class PlayerDataManager : MonoBehaviour, MMEventListener<SavePlayerDataEvent>, MMEventListener<LoadPlayerDataEvent>
    {
        ...
        {
            PlayerDataList.Add(new PlayerData("Player1", 0, 1, 10, 6, 10, 30, 1));
            PlayerDataList.Add(new PlayerData("Player2", 0, 1, 10, 6, 10, 30, 1));
        }
        ...
        public void OnMMEvent(LoadPlayerDataEvent e)
        {
            foreach (var data in PlayerDataList)
            {
                ...
                data.CoinsForNextLevel = PlayerPrefs.GetInt($"{data.PlayerID}_CoinsForNextLevel", 10);
